---
layout: default
title: Gunslinger
description: "Chaque jour, après le premier vote d’exécution, vous pouvez choisir publiquement un joueur ayant voté : cette personne meurt."
image: /images/Icon_gunslinger.png
---

<p align="left">
  <a href="/botc-fr-bambi/">
    <img src="../images/logogold.png" alt="Accueil BotC FR" width="300">
  </a>
</p>

<!-- Gunslinger -->

<div style="text-align:center; margin: 20px 0;">
  <img src="../images/Icon_gunslinger.png" alt="Gunslinger" width="360" style="border-radius:12px; margin-top:10px;">
  <br>
  <span style="color:#9b59b6; font-weight:bold; font-size:28px; letter-spacing:0.5px;">Gunslinger</span>
</div>

---

##  Informations

- **Type :** <a href="./voyageurs.html" style="color:#9b59b6; font-weight:bold; text-decoration:none;">Voyageur</a>  
- **Édition :** <a href="/botc-fr-bambi/trouble_brewing.html" style="color:#b58b52; font-weight:bold; text-decoration:none;">Trouble Brewing</a>  
- **Artiste :** <span style="color:#f5f5f5;">Aidan Roberts</span>

<p style="color:#f5f5f5; font-style:italic; font-size:17px; text-align:center;">
« Il est temps que quelqu’un prenne les choses en main. Ce quelqu’un... c’est moi. »
</p>

---

##  Apparaît dans

<p style="text-align:center; margin: 12px 0 6px;">
  <a href="/botc-fr-bambi/trouble_brewing.html" style="text-decoration:none;">
    <img src="../images/Logo_trouble_brewing.png" alt="Trouble Brewing" width="220" style="border-radius:8px;">
  </a>
</p>

<p style="text-align:center;">
   <a href="/botc-fr-bambi/trouble_brewing.html" style="color:#b58b52; font-weight:bold; text-decoration:none; font-size:22px;">Trouble Brewing</a>
</p>

---

##  Résumé

<p style="color:#f5f5f5; font-size:18px; line-height:1.7;">
<strong>« Chaque jour, après le premier vote d’exécution, vous pouvez choisir publiquement un joueur ayant voté : cette personne meurt. »</strong>
</p>

<p style="color:#f5f5f5; font-size:18px; line-height:1.7;">
Le <strong>Gunslinger</strong> tue une personne ayant voté pendant le premier vote d’exécution de la journée.
</p>

<ul style="color:#f5f5f5; font-size:18px; line-height:1.7;">
  <li>Une fois par jour, après le premier vote d’exécution, le Gunslinger peut <strong>désigner publiquement</strong> une personne ayant voté pour la faire mourir immédiatement.</li>
  <li>Capacité facultative.</li>
  <li>Jamais plus d’une utilisation par jour.</li>
  <li>Les <strong>exils</strong> ne sont pas concernés.</li>
</ul>

---

## Comment Conter

<p style="color:#f5f5f5; font-size:18px; line-height:1.7;">
Après le premier vote d’exécution de la journée, quand le décompte est annoncé, le Gunslinger peut déclarer qu’il utilise sa capacité et désigner une des personnes ayant voté. Cette personne <strong>meurt immédiatement</strong>. Le conteur ou la conteuse peut demander aux votants et votantes de relever la main pour clarifier si besoin.
</p>

---

##  Exemples

<ul style="color:#f5f5f5; font-size:18px; line-height:1.7;">
  <li>L’<a href="../tb_roles/imp.html" style="color:#d45b5b; font-weight:bold; text-decoration:none;">Imp</a> est nommé.  
  → Cinq personnes sur dix votent pour l’exécuter.  
  → Le Gunslinger choisit l’une des personnes ayant voté : elle meurt, mais l’Imp reste en attente d’exécution.</li>

  <li>Le <a href="../tb_roles/voleur.html" style="color:#9b59b6; font-weight:bold; text-decoration:none;">Voleur</a> est exilé.  
  Plus tard, le <a href="../tb_roles/majordome.html" style="color:#4ea3ff; font-weight:bold; text-decoration:none;">Majordome</a> est nommé et reçoit 1 vote.  
  C’est le premier vote du jour : le Gunslinger tue la personne qui a voté.</li>
</ul>

---

##  Conseils et astuces pour les <span style="color:#4ea3ff;">Bons</span>

<ul style="color:#f5f5f5; font-size:18px; line-height:1.7;">
  <li>Les morts par votre capacité <strong>ne sont pas des exécutions</strong> : la journée continue et le <a href="../tb_roles/croquemort.html" style="color:#4ea3ff; font-weight:bold; text-decoration:none;">Croque-Mort</a> ne voit rien.</li>
  <li>Utiliser votre pouvoir chaque jour augmente vos chances de faire tomber le <span style="color:#d45b5b;">Démon</span>.</li>
  <li>Observez qui évite de voter : les <span style="color:#d45b5b;">Sbires</span> et <span style="color:#d45b5b;">Démons</span> se méfient de vous.</li>
  <li>Excellent contre le <a href="../tb_roles/saint.html" style="color:#4ea3ff; font-weight:bold; text-decoration:none;">Saint</a> : il n’est pas exécutable, mais vous pouvez le tuer.</li>
  <li>Fin de partie à 3 : tirez avec prudence, sinon vous offrez la victoire au Mal.</li>
</ul>

---

##  Conseils et astuces pour les <span style="color:#d45b5b;">Maléfiques</span>

<ul style="color:#f5f5f5; font-size:18px; line-height:1.7;">
  <li>Tirez souvent… mais choisissez vos cibles intelligemment.</li>
  <li>Évitez de tuer votre <span style="color:#d45b5b;">Démon</span> (sauf combo utile avec la <a href="../tb_roles/femmeecarlate.html" style="color:#d45b5b; font-weight:bold; text-decoration:none;">Femme Écarlate</a>).</li>
  <li>Éliminez une <a href="../tb_roles/voyante.html" style="color:#4ea3ff; font-weight:bold; text-decoration:none;">Voyante</a> ou un <a href="../tb_roles/soldat.html" style="color:#4ea3ff; font-weight:bold; text-decoration:none;">Soldat</a> crédible.</li>
  <li>Intimidez : la menace de « tirer » influence les votes.</li>
  <li>À 3 vivants restants, tuez une personne non-Démon et laissez le chaos finir le travail.</li>
</ul>

---

<ul style="color:#f5f5f5; font-size:18px; line-height:1.7;">
  <li> <a href="/botc-fr-bambi/" style="color:#f5f5f5; font-weight:bold; text-decoration:none;">Retour à l’accueil</a></li>
  <li> <a href="./voyageurs.html" style="color:#9b59b6; font-weight:bold; text-decoration:none;">Retour aux Voyageurs</a></li>
</ul>
